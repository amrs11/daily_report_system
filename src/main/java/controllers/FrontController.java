...
public class FrontController extends HttpServlet {
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //該当するActionオブジェクトを作成（例：リクエストからパラメータaction=Employeeの場合、action.EmployeeActionオブジェクト）
            type = Class.forName(String.format("actions.%Action", actionString));

            //ActionBaseのオブジェクトにキャスト（例：action.EmployeeActionオブジェクト→action.ActionBaseオブジェクト）
            action = (ActionBase) (type.asSubclass(ActionBase.class)
                    .getDeclaredConstructor()
                    .newInstance());
        } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | SecurityException
                | IllegalArgumentException | InvocationTargetException| NoSuchMethodException e) {
            //リクエストパラメータに設定されている"action"の値が不正の場合、（例：action=XXXXXなど該当するActionクラスが無い場合
            //エラー処理を行うActionクラスを作成
